...
> **Note on the Python pipeline.** Python's **structural** and **data_contract** maps route through `PythonAdapter` (`ast`-based) — the same shared adapter path as Go/Java/TS. Python's **runtime** and **authority** maps still use dedicated builders (`runtime_builder.py` / `authority_builder.py`): their target-resolution, provenance tracking, and entry-point-merge logic is richer than the adapter and was deliberately not unified (see `docs/AUTONOMOUS_FIX_PLAN.md`). `PythonAdapter.extract_runtime` / `extract_writer_calls` exist but are not yet wired for Python. Reads (`open(p)` / `open(p, "r")` / `.read_text()` / `json.load` / `json.dumps`) are not writes.
